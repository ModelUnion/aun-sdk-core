...
describe('GroupE2EEManager.loadAllSecrets', () => {
  it('应加载所有 epoch 密钥并返回 Map<epoch, Buffer>', () => {
    const aunPath = mkdtempSync(join(tmpdir(), 'aun-meta-'));
    const client = new AUNClient({ aun_path: aunPath });

    const aid = 'test.aid.com';
    const groupId = 'group-1';

    // 设置 identity 使 _currentAid() 可用
    (client as any)._identity = { aid };
    ...
    // 存储两个 epoch
    if (ks.storeGroupSecretTransition) {
      ks.storeGroupSecretTransition(aid, groupId, {
        epoch: 1,
        secret: Buffer.from('secret-1').toString('base64'),
        commitment: 'commit-1',
        memberAids: [aid],
      ...
      ks.storeGroupSecretTransition(aid, groupId, {
        epoch: 2,
        secret: Buffer.from('secret-2').toString('base64'),
        commitment: 'commit-2',
        memberAids: [aid],
        oldEpochRetentionMs: 7 * 24 * 3600 * 1000,
      });
    ...
    const secrets = groupE2ee.loadAllSecrets(groupId);
    expect(secrets).toBeInstanceOf(Map);
    // 至少有 epoch 2（当前 epoch）
    expect(secrets.has(2)).toBe(true);
    expect(secrets.get(2)).toBeInstanceOf(Buffer);
  });
});
