...
export function nodeFromAny(input: unknown): NodeView {
  ...
  if (['folder', 'dir', 'directory'].includes(nodeType)) {
    return baseNode(raw, 'dir', keyToPath(raw.path));
  }
  if (['symlink', 'link'].includes(nodeType)) {
    return baseNode(raw, 'symlink', keyToPath(raw.path));
  }
  if (nodeType === 'mount') {
    return baseNode(raw, 'mount', keyToPath(raw.path));
  }
