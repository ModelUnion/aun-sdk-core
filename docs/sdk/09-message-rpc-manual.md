...
| `type` | string | 否 | `"text"` | 消息类型（应用层自定义） |
...
`payload` 是应用层 JSON 对象，服务端只做大小限制和透传，不按本节字段做强制校验。为了让不同客户端形成一致展示和处理习惯，建议采用以下松散约定：

- 外层 `type` 表示粗粒度消息类别；业务私有子类型放在 `payload.kind`，建议使用反向域名或产品前缀。
- 正文统一放在 `payload.text`；富文本格式用 `payload.format` 标注，如 `"plain"` / `"markdown"`。
- 会话、引用、线程、展示类元数据放在 `payload` 中，不改变传输信封。
- 大文件、二进制内容不直接嵌入 `payload`；先经 `storage.*` 上传，再携带对象引用。

| 外层 `type` | 推荐 payload 字段 | 说明 |
|------|------|------|
| `text` | `text`, `format`, `lang` | 普通文本或 Markdown |
| `file` | `text`, `attachments` | 普通文件，`text` 可作说明 |
| `image` | `text`, `attachments`, `alt`, `width`, `height` | 图片或图片集，附件中声明 MIME |
| `audio` | `attachments`, `duration_ms`, `transcript` | 音频消息，可附转写文本 |
| `video` | `text`, `attachments`, `thumbnail`, `duration_ms` | 视频消息，可附封面 |
| `link` | `url`, `title`, `description`, `thumbnail` | 链接或卡片预览 |
| `json` | `kind`, `data`, `schema` | 结构化数据，适合 Agent 间协作 |
| `notice` | `text`, `level`, `code` | 通知类消息，如状态变更 |
| `reaction` | `target`, `action`, `key` | 对已有消息添加或移除反应 |
| `tool_call` | `call_id`, `name`, `arguments` | 能力调用请求 |
| `tool_result` | `call_id`, `ok`, `result`, `error` | 能力调用结果 |
| `custom` | `kind`, `data` | 自定义消息；接收方不认识时应降级展示 |

公共辅助字段可按需放入 `payload`：

| 字段 | 类型 | 说明 |
|------|------|------|
| `chat_id` | string | 应用层会话标识；AUN P2P 不创建服务端会话生命周期 |
| `thread_id` | string | 话题、子线程或任务线程标识 |
| `reply_to` | object | 回复目标，推荐含 `message_id`、`seq`、`from` |
| `quote` | object | 展示用引用摘要，避免复制完整敏感原文 |
| `mentions` | array | 提及对象，推荐项为 `{aid, display, offset, length}` |
| `entities` | array | 文本实体，如链接、代码片段、时间范围 |
| `client_context` | object | 客户端自定义上下文，如窗口、任务、草稿来源 |

> 字段名建议使用 snake_case，如 `chat_id`；已有应用若使用 `chatId` 等命名，可在自己的应用层约定中保持一致。
...
{
    "text": "请查收附件",
    "chat_id": "dm-alice-bob",
    "reply_to": {"message_id": "prev-msg-id", "seq": 41},
    "attachments": [{
