...
| [message.thought.put](#messagethoughtput) | 写入某条 P2P 消息的思考内容 |
| [message.thought.get](#messagethoughtget) | 读取某条 P2P 消息的思考内容 |
...
写入当前发送者针对一条 P2P 消息的思考内容。它不是普通消息：不广播、不进 `message.pull`、不占用收件箱 `seq`，只通过 `message.thought.get` 主动读取。
...
存储键为 `sender_aid + peer_aid + reply_to.message_id`。其中 `sender_aid` 由认证态派生，`peer_aid` 来自 `to`。同一会话里每个 sender 保留最近 N 个 `reply_to.message_id` 对应的 head，N 由 Message 服务配置 `max_thought_heads_per_sender` 控制，当前默认值为 5；同一个 head 下可追加多条 thought item。
...
| `reply_to.message_id` | string | 是 | 被思考的 P2P 消息 ID，也是 thought head 的键 |
...
await client.call("message.thought.put", {
    ...
    "payload": {"type": "thought", "text": "先确认 Bob 的约束，再给最终答复"},
})
```
...
读取指定发送者针对指定 P2P 消息的当前思考内容。`get` 是查询操作，可重复调用；它不触发 push/pull、ack 或 replay 消费。
