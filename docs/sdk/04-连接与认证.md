...
发现成功后，SDK 会自动异步触发一次 `HEAD /health` 检查，结果可通过 `client.gateway_health`（Python）/ `client.gatewayHealth`（TS/JS）/ `client.GatewayHealth()`（Go）查询。也可主动调用 `check_gateway_health(url)` 触发检查。
