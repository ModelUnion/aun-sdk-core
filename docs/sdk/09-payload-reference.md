...
示例展示的是 `payload` 片段：P2P 完整请求仍需要在同级传入 `to`；群消息完整请求仍需要在同级传入 `group_id`；P2P 思考内容需要 `to` + `reply_to.message_id`；群思考内容需要 `group_id` + `reply_to.message_id`。文本、图片、文件、思考内容等业务消息类型只能放在 `payload.type`；`message.send.params.type` / `message.thought.put.params.type` / `group.send.params.type` / `group.thought.put.params.type` 是信封或封装类型，例如 SDK 加密发送时自动填充的 `e2ee.encrypted` / `e2ee.group_encrypted`。
...
| `thought` | 思考过程片段 | Agent 针对某条 P2P 或群消息的非广播思考内容 |
...
| `reply_to.message_id` | `message.thought.put/get.params` 和 `group.thought.put/get.params` | 思考内容的目标消息键；不要只放在 payload 内 |
...
`thought` 用于 Agent 暴露针对某条 P2P 或群消息的思考过程片段。它只应通过 `message.thought.put` 或 `group.thought.put` 发送，不作为普通 `message.send` / `group.send` 消息广播；有兴趣的客户端通过对应的 `*.thought.get` 主动读取。
...
`message.thought.put` / `group.thought.put` 的 `reply_to.message_id` 是顶层必填键，用于定位被思考的消息；`payload` 内如需展示引用摘要，可另行携带 `quote` 或 `client_context`，但不能替代顶层 `reply_to.message_id`。
