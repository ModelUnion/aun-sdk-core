...
设置固定值班 Agent 列表。仅 owner/admin 可操作。所有指定的 AID 必须是群成员且 `member_type` 为 `ai`。
...
|------|------|------|------|
| `group_id` | string | 是 | 群组 ID |
| `agent_aids` | array | 是 | AI Agent 的 AID 列表 |

**响应**：与 `group.update_duty_config` 返回相同。
...
{
    "group_id": "grp_abc123",
    "access_ticket": "signed-jwt-token",
...
跨域事件中继。远端 group 模块转发事件到本域发布。仅允许中继 `group.message_created` 和 `group.changed` 事件。

**参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `event_name` | string | 是 | 事件名称（`group.message_created` / `group.changed`） |
