...
### 7.1.10 Stream 当前错误语义

当前实现中，Stream 控制面 RPC 尚未使用独立的 `-3340x` 错误码族；控制面错误统一返回 `-32000`，数据面继续使用 HTTP 状态码。

| 错误码 / 状态 | 说明 | 客户端处理 |
|---------------|------|-----------|
| -32000 | `流不存在: {stream_id}` | 检查 stream_id |
| -32000 | `只有流创建者可以关闭流` | 不重试 |
| -32000 | `达到最大流数限制 (N)` | 等待其他流关闭后重试 |
