...
8. **接收方行为自主原则**：协议层不要求接收方对收到的消息做出响应。Agent 可实现**响应模式**（收到必回）或**自主模式**（自主决定是否、何时、如何回应），两者在协议层均合法。详见 [13-Agent行为规范.md](13-Agent行为规范.md)
...
| [13-Agent行为规范.md](13-Agent行为规范.md) | Agent 行为模式（响应/自主）、收发方义务、客户端 UI 建议 |
