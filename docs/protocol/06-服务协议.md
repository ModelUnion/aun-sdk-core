...
| `message.*` | 核心 | 点对点消息路由、E2EE V2 设备密钥/bootstrap | message 服务 | WebSocket |
...
| `proxy.*` | 扩展 | Service Proxy 服务列表控制面 | Gateway 本地处理 / service_proxy 查询 | WebSocket |
...
### 6.2 Service Proxy 服务列表控制面 `proxy.*`

`proxy.*` 是 AUN Service Proxy 的控制面命名空间，用于 provider 通过 Gateway 长连接声明自己能够通过 Service Proxy 暴露哪些服务。它不承载 HTTP/WS 数据面流量；真实访问数据面由 `service_proxy` 模块的独立 proxy-server 隧道处理。

#### 控制面与数据面

- **Gateway 控制面**：provider 的 AUN 长连接调用 `proxy.register_services` / `proxy.unregister_services` / `proxy.list_services`。Gateway 记录每个 provider 长连接声明的服务摘要，供 `service_proxy` 模块判断 provider 是否在线、是否声明目标服务。
- **proxy-server 数据面**：service-proxy-client 连接 `service_proxy` 模块的 `/ws/client` 后，认证成功后必须再次发送隧道消息 `register_services`。proxy-server 只向已在本地数据面连接并注册目标服务的 provider 隧道转发请求。
- **服务列表生命周期**：Gateway 控制面服务列表绑定 Gateway 长连接；proxy-server 数据面服务列表绑定 proxy-server 隧道连接。连接断开后对应服务列表必须清理。
- **一致性要求**：同一 provider AID 的多个实例可以同时在线，但它们注册的服务列表必须一致。这里的一致性以清洗后的服务摘要为准，包括 `service_name`、`service_type`、`visibility` 和非敏感 `metadata`。
- **粘性路由**：同一 provider AID 的多个实例若提供相同服务，proxy-server 必须对同一访问亲和键保持稳定路由；亲和键可由 requester AID、客户端 IP、provider AID 和服务名组合得到。

#### 服务摘要

服务摘要不得包含内网 endpoint、token、cookie、私钥、证书等敏感字段。实现必须在 Gateway 控制面和 proxy-server 数据面分别清洗摘要，不能依赖共享代码。
...
| 字段 | 必需 | 说明 |
|------|:----:|------|
| `service_name` | ✅ | 服务名，只允许小写字母、数字、`_`、`-`；不得使用 `api`、`health`、`proxy`、`ws` 等保留名 |
| `service_type` | ❌ | 服务类型，默认 `http`；可用于提示 `http` / `sse` / `websocket` / `mcp` / `file` 等 |
| `visibility` | ❌ | 访问策略，默认 `private`；当前实现支持 `public`、`private`、`aun-auth` |
| `metadata` | ❌ | 非敏感元数据；实现必须过滤 endpoint、url、token、authorization、cookie、secret、private_key、cert 等敏感键 |

#### `proxy.register_services`

注册当前 Gateway 长连接提供的 Service Proxy 服务列表。服务端必须以连接认证得到的 AID 作为 provider AID；客户端传入的 `provider_aid` 只能作为兼容字段或诊断字段，不能覆盖认证身份。
...
```json
{
  "jsonrpc": "2.0",
  "id": "reg-1",
  "method": "proxy.register_services",
  "params": {
...
注销当前 Gateway 长连接上的部分或全部 Service Proxy 服务。省略 `service_names` 时注销该连接全部服务。

```json
{
  "jsonrpc": "2.0",
  "id": "unreg-1",
  "method": "proxy.unregister_services",
  "params": {"service_names": ["fileshare"]}
}
```

响应包含 `removed` 和剩余 `count`。如果同一 provider AID 还有其他连接，部分注销导致服务列表不一致时，服务端应拒绝该操作。

#### `proxy.list_services`

列出当前 Gateway 长连接已注册的服务摘要。

```json
{
  "jsonrpc": "2.0",
  "id": "list-1",
  "method": "proxy.list_services",
  "params": {}
}
```

#### 内部查询 `gateway.query_proxy_services`

`gateway.query_proxy_services` 是 service_proxy 模块调用 Gateway 的内部模块 RPC，不是普通客户端公开方法。调用方必须是 `service_proxy` 模块。

请求参数：

| 参数 | 必需 | 说明 |
|------|:----:|------|
| `provider_aid` | ✅ | 被访问 provider AID |
| `service_name` | ❌ | 目标服务名；为空时只查询 provider 在线和服务列表概览 |

响应字段：

| 字段 | 说明 |
|------|------|
| `provider_aid_exists` | Gateway 通过 CA 证书查询确认 provider AID 是否存在；查询失败时为 `null` |
| `provider_online` | provider 是否存在 Gateway 长连接 |
| `has_registered_services` | 在线连接中是否已有 proxy 服务列表注册 |
| `service_registered` | 指定 `service_name` 是否已由在线 provider 注册 |
| `service_names` | 当前 provider 声明的服务名集合 |
| `connections` | 已注册服务列表的 Gateway 长连接摘要 |
| `matched_connections` | 注册了指定服务的 Gateway 长连接摘要 |

#### proxy-server well-known 发现

proxy-client 不得由应用外部传入、配置或硬拼 proxy-server 地址。连接 proxy-server 前必须先执行发现：

1. 优先请求 `https://{provider_aid}/.well-known/aun-proxy`。
2. 请求失败时回退 `https://proxy.{issuer}/.well-known/aun-proxy`。
3. 发现结果可缓存到 provider AID 本地 SQLite metadata，缓存 TTL 为 1 小时。
...
```json
{
  "issuer": "agentid.pub",
  "provider_aid": "alice.agentid.pub",
  "proxy_host": "proxy.agentid.pub",
  "https_url": "https://proxy.agentid.pub:19890",
...
proxy-client 连接 `ws_url` 时必须带 `Authorization: Bearer <access_token>`。`access_token` 优先复用本地 cached token；缓存缺失或过期时，必须通过 AUNClient 向 Gateway 完成两步登录取得新 token。

#### proxy-server 隧道注册

service-proxy-client 连接 `service_proxy` 模块的 `/ws/client` 后，认证成功后必须发送：
...
若同一 provider AID 的另一条 proxy-server 隧道已注册不同服务列表，proxy-server 必须返回：
...
proxy-server 处理 `https://proxy.{issuer}/{user_name}/{svc_name}/...` 或等价路由时，必须按以下顺序决策：

1. 本地 proxy-server 连接索引中存在 `(provider_aid, service_name)`：直接选择对应数据面连接；多连接时按亲和键稳定选择。
2. provider 已连接本 proxy-server 且已注册服务列表，但没有目标服务：返回 `service_not_registered`，不得 wakeup。
3. provider 未连接本 proxy-server 时，查询 Gateway 控制面：
   - `provider_aid_exists=false`：返回 `provider_aid_not_found`。
   - `provider_aid_exists=null`：返回 `provider_aid_check_failed`。
   - `provider_online=false`：返回 `provider_offline`。
   - `provider_online=true` 且 `service_registered=false`：返回 `service_not_registered`，不得 wakeup。
   - `provider_online=true` 且 `service_registered=true`：通过 Gateway 向注册了目标服务的 provider 长连接发送 `app.service_proxy.wakeup`。
4. wakeup 已投递但本 proxy-server 仍未等到目标服务注册时，返回 `provider_wakeup_timeout`。
...
| `type` | ❌ | 信封/封装类型，普通业务消息无需填写；SDK V2 加密发送时自动使用 `e2ee.p2p_encrypted` |
...
| `limit` | ❌ | 单次返回上限（默认 50，最大 50；`pull_max_limit` 配置只能进一步收紧） |
...
> 当前 V2 主路径见 [E2EE_V2消息通信时序图](../sdk/E2EE_V2消息通信时序图.md)、[05-E2EE加密通信](../sdk/05-E2EE加密通信.md) 和 [08-AUN-E2EE-Group.md](08-AUN-E2EE-Group.md)。[08-AUN-E2EE.md](08-AUN-E2EE.md) 仅保留 legacy P2P 信封说明。本节仅保留摘要。

**核心原则**：

- E2EE 加解密**完全由客户端实现**，无需在线协商
- 服务端只做认证、路由、设备密钥/群状态查询、密文存储和事件通知，对明文内容完全透明
- P2P 加密消息通过 `message.send` 承载 V2 `e2ee.p2p_encrypted` envelope；群加密消息通过 `group.v2.send` 承载 V2 `e2ee.group_encrypted` envelope
- 当前主路径为 V2 多设备 wrap：每条消息一把 `master_key`，按 recipient 设备分别生成 `3DH` / `1DH` wrap，并由接收端验证 `sender_signature`、AAD 和 recipient proof / digest

**服务端兼容性要求**：

- 接受并转发 V2 E2EE envelope；兼容旧 `encrypted: true` 标志
- 在离线队列和历史消息中存储密文 payload
- 不对密文 payload 施加额外的语义解析要求
- 提供 `message.v2.put_peer_pk`、`message.v2.bootstrap`、`group.v2.put_group_pk`、`group.v2.bootstrap` 等 V2 设备密钥和 bootstrap RPC

**互操作要求**：

跨语言、跨平台 SDK 的当前互操作以 E2EE V2 多设备 wrap 为准：P2P 参见 SDK V2 时序和各语言 `v2/e2ee` 实现，群组参见 [08-AUN-E2EE-Group.md](08-AUN-E2EE-Group.md)。旧 `e2ee.encrypted` 信封互操作才查 [08-AUN-E2EE.md](08-AUN-E2EE.md)。
