...
| `fanout` | 广播到在线实例 | 数据库 + 文件存储 | TTL 过期前持久保存（默认 24 小时，可由服务端配置调整） | 离线送达、历史查询、多实例同步接收 |
...
- 若某个外域查询失败，该域内 AID 回落为 `false`，不影响整体响应
...
跨域消息路由用于不同 Issuer 域之间的消息转发，已在 Gateway federation 通道上落地。

**当前实现事实**：

- 本域服务通过 `gateway.forward_federation` / `gateway.forward_federation_batch` 发起跨域 RPC 中继
- Gateway federation client 通过 `https://{issuer}/.well-known/aun-gateway` 发现远端 Gateway 地址
- 远端 Gateway 通过 `federation.hello` 完成双向认证后，接收 `federation.relay` / `federation.relay_batch`
- `message.send`、`message.query_online`、`storage.*`、`group.*` 等服务已复用该通道进行跨域转发
- 服务端向被中继的 RPC 注入 `_federation` 上下文，并校验调用方 AID 的 issuer 与来源 issuer 一致

**当前约束**：

- 这是 Gateway 级 federation 中继通道，不是独立的 `message.*` 子协议扩展
- 跨域转发的可用 namespace 受 Gateway 白名单控制

**错误语义**：

| 错误码 | 说明 |
|--------|------|
| `-32010` | federation 连接失败 |
| `-32011` | 跨域转发超时 |
| `-32012` | 运行时拒绝或协议错误 |
