...
- `docs/INDEX.md` L7-38：根级文档地图。

### Step 2：按主题读根级 Layer 2

- 测试与 E2E：`docs/INDEX.md` L44-51。
- SDK 使用与协议：`docs/INDEX.md` L53-66。
- Service Proxy 与服务暴露：`docs/INDEX.md` L68-73。
- 诊断与可观测性：`docs/INDEX.md` L75-81。
- E2EE 与跨语言一致性：`docs/INDEX.md` L83-87。

### Step 3：需要判断文档价值时读 Layer 3 摘要

- `docs/INDEX.md` L91-179：重点文档摘要。
...
| SDK API、RPC、E2EE、Storage VFS、Group resources pending_ops、Collab GC/reflog/reset 使用细节 | `docs/sdk/AUN_DOCS_GUIDE.md` |
...
| group.fs POSIX 化群文件系统详细设计、memberdata 服务端映射、SDK 数据面编排和 `cp/mv` 语义 | `docs/aun-fs/group-storage/group.fs-POSIX化详细设计.md` L1-609 |
| group.fs POSIX 化分阶段实施计划、每阶段 IPO、Step-by-step、用例集、Checklist、验收和回滚 | `docs/aun-fs/group-storage/group.fs-POSIX化分阶段实施计划.md` L1-740 |
| 群存储 group-storage 终态设计、group_aid 命名空间、成员 `memberdata` 挂载和 `group.resources.*` 门面重映射 | `docs/aun-fs/group-storage/2026-06-10-group-storage-重构设计.md` L9-83、L86-173、L177-230、L234-360 |
| 群存储 group-storage 0-10 阶段 TDD 实施记录、CLI/E2E 收口、Docker 实跑子阶段和单域/双域 E2E 通过记录 | `docs/aun-fs/group-storage/2026-06-10-group-storage-重构实施计划.md` L1-63、L64-566、L567-600、L601-679 |
