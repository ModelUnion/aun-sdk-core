...
| 单域 Docker 测试容器和命令 | `docs/aun测试运行指南.md` L105-494 |
| P2P bench 标准流程、bench-pairs16、uvloop、Gateway mock 延迟和 perf-trace | `docs/aun测试运行指南.md` L318-385 |
| 跨语言容器 E2E 执行方式 | `docs/aun测试运行指南.md` L139-263 |
| 双域 federation 测试容器和命令 | `docs/aun测试运行指南.md` L496-694 |
| 何时 rebuild / restart | `docs/aun测试运行指南.md` L696-739 |
| 测试故障排查 | `docs/aun测试运行指南.md` L742-779 |
| 跨语言容器 E2E 背景与目标 | `docs/design/跨语言容器E2E测试方案.md` L5-35 |
| E2EE V2 1DH/per-AID wrap 方案 | `docs/design/E2EE_V2简化为1DH加Per-AID_Wrap方案.md` |
| E2EE V2 当前 P2P/Group 主链路 | `docs/sdk/E2EE_V2消息通信时序图.md` |
...
| agent.md 远程缓存目标与字段 | `docs/agent.md/远程agent.md缓存与etag透传方案.md` L5-127 |
...
| AUN FS POSIX 覆盖现状、过期状态修正、缺口优先级和 step-by-step TDD 补全计划 | `docs/aun-fs/aun-fs-posix-coverage.md` L14-224、`docs/aun-fs/aun-fs-completion-plan.md` L9-66、L93-142、L144-257、L259-353、L355-608、L698-790 |
| group.fs POSIX 化群文件系统详细设计、memberdata 服务端映射、SDK 数据面编排、群自有区角色 ACL 授权/撤销/查询、`parents` 和 `cp/mv` 语义 | `docs/aun-fs/group-fs/group.fs-POSIX化详细设计.md` L1-611 |
| 群自有区 owner 默认写、admin 需 owner 显式 `role:admin` ACL 授权与撤销、Storage 写审计和 TDD 计划 | `docs/aun-fs/group-fs/群自有区角色ACL写权限实施方案.md` |
| `memberdata` / `group_data` 系统目录保护、`group_data` 目录树隐藏与读下载/写保护边界、Group FS 授权路径和配额归属 | `docs/protocol/16-系统目录保护方案.md` |
