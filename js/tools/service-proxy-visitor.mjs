...
await checkHTTP('GET', `${proxyBase}/${user}/rest/headers`, { Host: proxyHost, 'x-trace-id': 'js-visitor' });
...
console.log('JS Service Proxy visitor 检查完成: PASS');
...
async function checkWS(targetURL) {
  const ws = new WebSocket(targetURL, ['chat.v1']);
