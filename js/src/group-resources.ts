...
export interface GroupResourcesRpcClient {
  call(method: string, params?: RpcParams): Promise<RpcResult>;
}

export type GroupResourceParams = RpcParams;

interface GroupResourcesAidStore {
  load(aid: string): unknown | Promise<unknown>;
}

type GroupResourcesSignerClient = GroupResourcesRpcClient & {
  connect(options?: GroupResourceParams): Promise<void>;
...
interface GroupResourcesOptions {
  ...
  connectOptions?: GroupResourceParams;
...
const DEFAULT_SIGNER_CONNECT_OPTIONS: GroupResourceParams = {
...
export class GroupPendingOpsPartialFailure extends Error {
  ...
  readonly failedOp: GroupResourceParams;
  readonly storageResults: GroupResourceParams;
  readonly opResults: RpcResult[];
  readonly compensationResults: GroupResourceParams;
  readonly compensationErrors: GroupResourceParams[];

  constructor(message: string, details: {
    failedIndex: number;
    failedOp: GroupResourceParams;
    storageResults: GroupResourceParams;
    opResults: RpcResult[];
    compensationResults: GroupResourceParams;
    compensationErrors: GroupResourceParams[];
    cause?: unknown;
  }) {
    super(message);
...
function stripNil(params?: GroupResourceParams | null): GroupResourceParams {
  const out: GroupResourceParams = {};
...
function asGroupResourceParams(value: unknown): GroupResourceParams {
  return isRecord(value) ? { ...(value as GroupResourceParams) } : {};
}
...
export class GroupResourcesFacade {
  ...
  ): Promise<GroupResourcesRpcClient> {
   ...
   */
  private async memberdataNamespaceKey(params: GroupResourceParams): Promise<string> {
  ...
  private resolveMemberdataTarget(groupKey: unknown, resourcePath: unknown): [string, string] | null {
    ...
    if (!selfAid || !sameAid(slotAid, selfAid)) return null;
  ...
  private async resolveMemberdataTargetForParams(params: GroupResourceParams): Promise<[string, string] | null> {
  ...
  private baselineDirs(params: GroupResourceParams): string[] {
  ...
  async put(params?: GroupResourceParams | null): Promise<RpcResult> {
    ...
    if (target) {
      ...
      const storageParams: GroupResourceParams = {
        owner_aid: ownerAid,
  ...
  async createFolder(params?: GroupResourceParams | null): Promise<RpcResult> {
    const merged = stripNil(params);
    const target = await this.resolveMemberdataTargetForParams(merged);
    if (target) {
  ...
  listChildren(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('list_children', params);
  }

  rename(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('rename', params);
  }

  move(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('move', params);
  }

  mountObject(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('mount_object', params);
  }

  unmount(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('unmount', params);
  }

  resolvePath(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('resolve_path', params);
  }

  get(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('get', params);
  }

  list(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('list', params);
  }

  update(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('update', params);
  }

  getAccess(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('get_access', params);
  }

  resolveAccessTicket(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('resolve_access_ticket', params);
  }

  async delete(params?: GroupResourceParams | null): Promise<RpcResult> {
    const merged = stripNil(params);
    const target = await this.resolveMemberdataTargetForParams(merged);
    if (target) {
      const [ownerAid, path] = target;
  ...
  namespaceReady(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('namespace_ready', params);
  }

  confirm(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('confirm', params);
  }

  confirmMount(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('confirm_mount', params);
  }

  getDf(params?: GroupResourceParams | null): Promise<RpcResult> {
    return this.call('get_df', params);
  }

  async initializeNamespace(
    params: GroupResourceParams | null = {},
    options: GroupResourcesOptions | null = {},
  ): Promise<RpcResult> {
    const input = params ?? {};
    const opts = options ?? {};
    ...
    const folderIds: GroupResourceParams = {};
    ...
    try {
      ...
      for (const dir of dirs) {
        const mkdirParams: GroupResourceParams = {
          owner_aid: groupAid,
          bucket,
          path: dir,
          parents: true,
  ...
  async executePendingOps(
    ...
    const results: GroupResourceParams = {};
    const opResults: RpcResult[] = [];
    const successfulOps: Array<{ op: GroupResourceParams; confirmKey: string; index: number }> = [];
    let lastConfirmKey = '';
    let lastResult: RpcResult | undefined;
    const signerCache = new Map<string, GroupResourcesSignerClient>();

    const runCompensations = async (): Promise<{ compensationResults: GroupResourceParams; compensationErrors: GroupResourceParams[] }> => {
      ...
      const compensationResults: GroupResourceParams = {};
      const compensationErrors: GroupResourceParams[] = [];
    ...
    try {
      ...
      const confirmParams: GroupResourceParams = {
        ...baseConfirmParams,
