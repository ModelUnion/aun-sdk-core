...
describe('P5 StorageVFS Browser SDK 契约', () => {
  ...
  it('read/list/stat 支持 token 透传并默认走 storage.fs.*', async () => {
    ...
    expect(Array.from(await storage.readBytes('/docs/a.txt', { token: 'tok' }))).toEqual(Array.from(new TextEncoder().encode('hello')));
  ...
  it('readBytes 支持 offset/limit 范围读取参数', async () => {
    ...
    expect(Array.from(await storage.readBytes('/docs/a.txt', { token: 'tok', offset: 1, limit: 3 }))).toEqual(Array.from(new TextEncoder().encode('hello')));
  ...
  it('downloadFile 返回实际 SHA256 并校验服务端哈希', async () => {
    ...
    expect(Array.from(downloaded.data ?? [])).toEqual(Array.from(new TextEncoder().encode('hello')));
...
describe('P5 StorageLowLevel Browser SDK 契约', () => {
