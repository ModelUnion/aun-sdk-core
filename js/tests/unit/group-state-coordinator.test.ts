...
async function cacheStateSignature(client: Record<string, any>, args: {
  groupId: string;
  actorAid: string;
  stateVersion: number;
  stateHash: string;
  ...
  const sigBytes = base64ToBytes(args.signatureB64);
  const frames: Uint8Array[] = [];
  for (const part of [actorBytes, payloadBytes]) {
    frames.push(encoder.encode(`${part.byteLength}:`), part, encoder.encode(';'));
  }
  frames.push(sigBytes);
  const cacheInput = new Uint8Array(frames.reduce((total, frame) => total + frame.byteLength, 0));
  let offset = 0;
