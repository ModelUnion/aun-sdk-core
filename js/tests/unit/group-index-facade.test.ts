...
async function baseIndex(signer: ReturnType<typeof fakeSigner>, seed: string) {
...
async function indexWithEntries(signer: ReturnType<typeof fakeSigner>, entries: Array<Record<string, any>>) {
  return buildSignedGroupIndex({
    groupAid: 'g-team.example.test',
    entries,
    signer,
...
class FakeClient {
  ...
  cachedGroupEtag = '';
  cacheCalls: Array<{ groupAid: string; settings: Record<string, any>; options?: Record<string, any> }> = [];
  ...
  cacheGroupIndexSettings(_groupAid: string, settings: Record<string, any>, options?: { entries?: Array<Record<string, any>>; etag?: string; groupIndex?: unknown }): void {
    this.cacheCalls.push({ groupAid: _groupAid, settings, options });
...
describe('browser GroupFacade.updateGroupIndex', () => {
  ...
  it('sends expected_index_etag and a signed group.index in the same settings update', async () => {
    ...
    const verified = await verifyGroupIndex(setCall?.settings['group.index'].body, client.currentAid);
  ...
  it('getGroupIndex fetches remote index and marks it fresh', async () => {
    const signer = fakeSigner();
    const remoteIndex = await baseIndex(signer, 'remote');
    ...
    expect(client.cacheCalls[0].options?.groupIndex).toEqual(remoteIndex);
    expect(client.calls).toEqual([
  ...
  it('getGroupIndex rejects a tampered remote index before updating cache', async () => {
    const signer = fakeSigner();
    const remoteIndex = tamperIndex(await baseIndex(signer, 'remote'));
    const client = new FakeClient([remoteIndex]);
    client.currentAid = signer;
  ...
  it('getRules caches settings under canonical and requested group ids', async () => {
    ...
    expect(client.cacheCalls.map((item) => item.groupAid)).toEqual(['g-team.example.test', 'legacy.remote.example']);
  });
  ...
  it('reloads index and retries once when CAS reports etag conflict', async () => {
    const signer = fakeSigner();
    const oldIndex = await baseIndex(signer, 'old');
    const newerIndex = await baseIndex(signer, 'newer');
