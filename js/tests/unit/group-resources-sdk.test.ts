...
describe('JS SDK facade API 契约', () => {
  ...
  it('client.group.resources.* 映射公开 group.resources RPC 方法', async () => {
    const client = new FakeClient();
    ...
    await resources.get({ group_id: 'g1', resource_id: undefined, resource_path: 'docs/a.txt', include_status: null });
    ...
    expect(client.calls[8]?.params).toEqual({ group_id: 'g1', resource_path: 'docs/a.txt' });
  ...
  it('initializeNamespace 传入 aidStore 时用 group_aid 短连接执行 storage RPC', async () => {
    ...
    await resources.initializeNamespace(
      ...
      {
        aidStore: {
          load: vi.fn().mockResolvedValue({ ok: true, data: { aid: { aid: 'team.agentid.pub' } } }),
        },
  ...
  it('executePendingOps 传入 aidStore 时按 sign_as 选择短连接执行 storage RPC', async () => {
    ...
    const aidStore = {
      load: vi.fn(async (aid: string) => ({ ok: true, data: { aid: { aid } } })),
    };
...
describe('memberdata 透明路由（成员挂载区映射到成员自己 storage 空间）', () => {
  // 协议约定：memberdata/{self_aid}/{rest} 的写操作 SDK 透明映射为
  // owner_aid=self_aid, object_key={self_aid}/{group_id}/{rest}，成员自己 AID 直连 storage.*。
  // 命中本人槽位才路由；他人槽位/群自有区走原 group.resources.* 流程。
  type ResolveFn = (groupId: unknown, resourcePath: unknown) => [string, string] | null;

  function memberFacade(aid: string): { resources: GroupResourcesFacade; client: FakeClient } {
    const client = new FakeClient();
    client.aid = aid;
    const resources = new GroupResourcesFacade(client);
  ...
  function resolve(resources: GroupResourcesFacade): ResolveFn {
    return (resources as unknown as { resolveMemberdataTarget: ResolveFn }).resolveMemberdataTarget.bind(
      resources,
    );
  }

  it('resolveMemberdataTarget 命中本人槽位时映射为 owner_aid + {self_aid}/{group_id}/{rest}', () => {
    const { resources } = memberFacade('alice.agentid.pub');
    expect(resolve(resources)('g-team.agentid.pub/team', 'memberdata/alice.agentid.pub/docs/x.txt')).toEqual([
      'alice.agentid.pub',
      'alice.agentid.pub/g-team.agentid.pub/team/docs/x.txt',
    ]);
  });

  it('resolveMemberdataTarget 槽位根路径映射为 group_id 本身', () => {
    const { resources } = memberFacade('alice.agentid.pub');
    expect(resolve(resources)('g-team.agentid.pub/team', 'memberdata/alice.agentid.pub')).toEqual([
      'alice.agentid.pub',
      'alice.agentid.pub/g-team.agentid.pub/team',
    ]);
  });

  it('resolveMemberdataTarget 群自有区返回 null', () => {
    const { resources } = memberFacade('alice.agentid.pub');
    expect(resolve(resources)('g-team.agentid.pub/team', 'announce/a.txt')).toBeNull();
  });

  it('resolveMemberdataTarget 他人槽位返回 null', () => {
    const { resources } = memberFacade('alice.agentid.pub');
    expect(resolve(resources)('g-team.agentid.pub/team', 'memberdata/bob.agentid.pub/x')).toBeNull();
  });

  it('resolveMemberdataTarget AID 大小写不敏感', () => {
    const { resources } = memberFacade('Alice.AgentID.pub');
    const target = resolve(resources)('g-team.agentid.pub/team', 'memberdata/alice.agentid.pub/x');
  ...
  it('put 命中本人 memberdata 时路由到 storage.put_object', async () => {
    const { resources, client } = memberFacade('alice.agentid.pub');
