...
func TestGroupAIDLoadsLegacyGroupStateKey(t *testing.T) {
	ks := testNewLocalTokenStore(t)
	aid := "group-aid.agentid.pub"
	legacyGroupID := "group.agentid.pub/room-123"
	groupAID := "room-123.agentid.pub"

	if err := ks.SaveGroupState(aid, legacyGroupID, 7, "state-hash-legacy", 3, "[]", "{}"); err != nil {
		t.Fatalf("SaveGroupState 失败: %v", err)
	}

	state, err := ks.LoadGroupState(aid, groupAID)
	if err != nil {
		t.Fatalf("LoadGroupState 失败: %v", err)
	}
	if state == nil || state.GroupID != legacyGroupID || state.StateHash != "state-hash-legacy" {
		t.Fatalf("state legacy lookup 不匹配: %#v", state)
	}
}
