...
class AUNClient:
    ...
    def _log_e2ee_error(self, stage: str, group_id: str, aid: str, exc: Exception) -> None:
        ...
        try:
            ...
            _client_log.debug("发布 e2ee 编排事件失败: %s", _pub_exc)
    ...
    async def _fill_group_gap(self, group_id: str) -> None:
        ...
        try:
            ...
            self._gap_fill_done.discard(dedup_key)
    ...
    def _build_rotation_signature(self, group_id: str, current_epoch: int, new_epoch: int = 0) -> dict[str, str]:
        ...
        try:
            ...
            _client_log.debug("构建轮换签名失败: %s", _sig_exc)
            return {}
        ...
        try:
            while not self._closing:
                ...
                try:
                    ...
                    _client_log.debug("epoch 轮换失败: %s", _exc)
        except asyncio.CancelledError:
            raise
        ...
        try:
            while not self._closing:
                ...
                if len(self._gap_fill_done) > 10000:
                    ...
                    to_remove = list(self._gap_fill_done)[:excess]
                    self._gap_fill_done -= set(to_remove)
                # e2ee prekey 缓存
