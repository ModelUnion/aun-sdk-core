...
环境变量（可选）：
  AUN_DATA_ROOT  — 数据存储根目录，默认 ~/.aun/examples
"""
...
# 每次进程启动生成一个随机后缀，避免多用户 AID 冲突
RUN_ID = random.randint(1000, 9999)

_DATA_ROOT = Path(
    os.environ.get("AUN_DATA_ROOT", "")
    or str(Path.home() / ".aun" / "examples")
)


def make_client(label: str) -> AUNClient:
    """创建一个 AUNClient 实例。label 用于隔离存储目录。"""
