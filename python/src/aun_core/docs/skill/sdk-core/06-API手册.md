...
**半状态恢复**：如果本地已有密钥但证书缺失（如之前注册中断），`register_aid()` 会自动检查服务端状态，若密钥对匹配则恢复证书。
...
| 异常类型 | 说明 |
|----------|------|
| `IdentityConflictError` | AID 已被其他密钥注册（继承自 `AuthError`） |
| `AuthError` | 其他认证相关错误 |
...
| TypeScript / JS | `await client.auth.registerAid({ aid: '...' })` |
| Go | `client.Auth.RegisterAID(ctx, params)` |
