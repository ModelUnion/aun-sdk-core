...
from aun_core.errors import AUNError, ConnectionError, AuthError, IdentityConflictError
...
async def create_client(aid: str) -> tuple[AUNClient, dict]:
    """创建客户端 → 加载或注册 AID → 认证 → 返回 (client, auth)"""
...
> **默认行为**：SDK 默认启用 P2P E2EE；Group E2EE 为必选能力并固定启用。`message.send` 和 `group.send` 默认加密发送；如需发送明文，显式传入 `encrypt=False`。`slot_id` 和 `delivery_mode` 只在 `connect(...)` 阶段传入。
...
2. **加载或注册 AID** - `load_identity_or_none()` → `register_aid()`
...
> **注意**：`authenticate()` 不再自动注册。正确流程为 `register_aid()` → `authenticate()` → `connect()`。

### 流控机制

SDK 内置 RPC 流控，防止并发请求过载：

- **全局 inflight 上限**：16 个并发 RPC
- **后台任务 inflight 上限**：8 个并发 RPC
- **Pull Gate**：同一 namespace/group 的 pull 请求串行化，避免重复拉取
- **队列超时**：排队过久时抛出 `TimeoutError`
