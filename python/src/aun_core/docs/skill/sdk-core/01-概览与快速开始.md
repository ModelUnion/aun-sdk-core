...
| **E2EE** | ECDH 会话协商、AES-256-GCM 加解密、会话管理 |
| **事件分发** | 服务端推送事件的订阅和分发 |
...
SDK 发送加密消息只需传 `encrypt=True`：

```python
await client.call("message.send", {
    "to": peer_aid, "payload": {"text": "secret"}, "encrypt": True,
})
```
...
| `root_ca_path` | str | — | 可选的额外 Root CA 证书 bundle 路径。SDK 默认内置根证书；认证第一阶段会使用本地信任根校验服务端 `identity_cert` 链，并通过 Gateway `/pki/chain` 获取缺失的中间 CA 证书，同时通过签名 CRL / OCSP 校验证书状态 |
