...
`message.send.params.payload` 的统一业务负载格式见 [消息Payload参考约定](../../sdk-core/消息Payload参考约定.md)。完整 P2P 请求仍在 `payload` 同级传入 `to`；业务类型放在 `payload.type`，不要与 `message.send.params.type` 信封/封装类型混用。
