...
async def test_p2p_payload_gap_fill():
    ...
    try:
        ...
        if not msgs:
            _fail("p2p_payload_gap_fill", "Bob 未收到任何消息")
            return

        first_seq = _seq_of(msgs[0])
        print(f"  [INFO] Bob 收到第一条消息 seq={first_seq}")

        # 3. Alice 再发送第 4、5 条消息
        await _sdk_send(alice, _BOB_AID, "msg4")
        await asyncio.sleep(0.2)
        await _sdk_send(alice, _BOB_AID, "msg5")

        # 4. Bob 直接拉取 seq=5（跳过 2/3/4），模拟 push 带 payload 的场景
        # SDK 应该检测到 gap 并自动补洞
        await asyncio.sleep(1.0)  # 等待服务端处理

        # 5. Bob 拉取所有消息
        all_msgs = await _sdk_recv_push_after(bob, _ALICE_AID, after_seq=0, timeout=2.0)

        # 6. 验证：应该收到 5 条消息，seq 连续
        if len(all_msgs) != 5:
            _fail("p2p_payload_gap_fill", f"期望 5 条消息，实际 {len(all_msgs)} 条")
            return

        seqs = [_seq_of(m) for m in all_msgs]
        ...
        payloads = [m.get("payload") for m in all_msgs]
...
async def test_group_payload_gap_fill():
    ...
    try:
        ...
        group_msgs = await bob.call("group.pull", {"group_id": group_id, "after_seq": 0, "limit": 50})
        msgs = group_msgs.get("messages", [])
        if not msgs:
            _fail("group_payload_gap_fill", "Bob 未收到任何群消息")
            return

        first_seq = _seq_of(msgs[0])
        print(f"  [INFO] Bob 收到第一条群消息 seq={first_seq}")

        # 4. Alice 再发送第 4、5 条群消息
        await alice.call("group.send_message", {
            "group_id": group_id,
            "payload": "group_msg4",
            "encrypt": True
        ...
        # 5. Bob 拉取所有群消息（SDK 应该自动补洞）
        await asyncio.sleep(1.0)
        all_group_msgs = await bob.call("group.pull", {"group_id": group_id, "after_seq": 0, "limit": 50})
        ...
        # 过滤出 Alice 发送的消息
        alice_msgs = [m for m in all_msgs if m.get("from") == _ALICE_AID]
        if len(alice_msgs) != 5:
            _fail("group_payload_gap_fill", f"期望 Alice 发送 5 条消息，实际 {len(alice_msgs)} 条")
            return

        seqs = [_seq_of(m) for m in alice_msgs]
        expected_seqs = list(range(first_seq, first_seq + 5))
        if seqs != expected_seqs:
            _fail("group_payload_gap_fill", f"seq 不连续：期望 {expected_seqs}，实际 {seqs}")
            return

        payloads = [m.get("payload") for m in alice_msgs]
